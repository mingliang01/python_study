...
class Main(QMainWindow, Ui_MainWindow):
    ...
    def query_ticketing_analysis_click(self):
        ...
        today_car_list.clear()  # 清空今天列车信息
        three_car_list.clear()  # 清空三天列车信息
        five_car_list.clear()  # 清空五天列车信息
        today_list.clear()
        three_list.clear()
        five_list.clear()
        ...
        if get_from != "" and get_to != "" :
            ...
            if get_from in stations and get_to in stations :
                ...
                query_ticketing_analysis(today, from_station, to_station,1)  # 发送查询请求,并获取返回的信息
                query_ticketing_analysis(three_day, from_station, to_station,3)  # 发送查询请求,并获取返回的信息
                query_ticketing_analysis(five_day, from_station, to_station,5)  # 发送查询请求,并获取返回的信息
        ...
        for train_number in self.info_table:
            ...
            for today in today_list:
                ...
                if train_number[0] in today:
                    is_today_ture = True  # 存在就进行标记
                    # train_number_list.append(train_number[0])    # 将车次添加至列表中
            ...
            if is_three_ture == False:  # 如果三天内没有某一趟列车，说明该车次无票为0
                number_list.append(0)
            is_five_ture = False  # 判断三天是否存在某趟列车的标记
    ...
    def query_time_click(self):
        ...
        if station in stations_time:  # 判断要搜索的站名是否存在
            ...
            for n in range(len(name_lit)):
                ...
                self.label.setText(name_lit[n]+'站'+'      '+time_list[n])   # 设置显示站名与起售时间
...
def show_MainWindow():
    ...
    # 卧铺售票分析查询按钮指定单击事件的方法
    main.pushButton_analysis_query.clicked.connect(main.query_ticketing_analysis_click)
    # 起售时间查询按钮指定单击事件的方法
    main.pushButton_time_query.clicked.connect(main.query_time_click)
